...
def webhook():
    ...
    if data.get('type') == 'document':
        ...
        if user_response.status_code == 404:
            send_message(phone_number, f"Your phone number is not registered on our website. Please register at {app.config['WEBSITE_LINK']} and upload files.")
        else:
                ...
                if media_response.status_code == 200:
                    ...
                    try:
                        ...
                        with open(temp_file_path, 'rb') as file_to_upload:
                            files = {'file': (file_name, file_to_upload, 'application/pdf')}
                            upload_response = requests.post(
                                app.config['FASTAPI_UPLOAD_URL'],
                                files=files,
                                params={'phone_number': phone_number_last_10_digits}
                            )
                            logging.info(f"File upload response: {upload_response.status_code}, {upload_response.text}")
                            if upload_response.status_code == 200:
                                ...
                                logging.error(f"Failed to upload file: {upload_response.status_code}, {upload_response.text}")
