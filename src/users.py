...
def login_user(userLogin: UserLogin, db: Session = Depends(get_db)):
    logging.info(f"Attempting login for user: {userLogin.username}")

    # Check if username or email exists in database
    user = db.query(User).filter(
        (User.username == userLogin.username) | (User.email == userLogin.username)
    ).first()
    if not user or not pwd_context.verify(userLogin.password, user.password):
        logging.warning(f"Login failed for user: {userLogin.username}")
        raise HTTPException(status_code=400, detail="Incorrect username or password")
    
    logging.info(f"User logged in successfully: {userLogin.username}")
